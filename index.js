...
window.addEventListener('scroll', () => {
	...
	parallax.forEach(layer => {
		// if (layer.className !== 'parallax layer1') return
		let lation = layer.getAttribute('data-speed') * (value)
		layer.setAttribute('style', 'transform: translate3d(0px, ' + lation + 'px, 0px)')
		// layer.setAttribute('style', 'transform: translate3d(' + lation + 'px, 0px, 0px)')
		// layer.setAttribute('style', 'transform: translate3d(0px, 0px, ' + lation + 'px)')
	})
})
...
function selectPage(e) {
	...
	switch (target) {
		...
		case 'still':
			...
			break;
		case 'faded':
			sheets = prepareSong(fadedStr)
			keySig(7, major)
			songSig = 7
			setChords('2e_2g_2b 2c_2e_2g 2g_2b_3d 2d_2f-_2a')
			keyCounter.innerHTML = sheets.length - pipes
			sigCounter.innerHTML = 'G major'
			break;
		default:
...
const fadedStr = `G3 G3 G3 B3 | E4 E4 E4 D4 | B3 B3 B3 B3 | F3 F3 F3 E3 |
